...
public class LDAP {
  ...
  public func search(base:String = "", filter:String = "(objectclass=*)", scope:Scope = .BASE, limit: Int32 = 5, completion: @escaping ([String:[String:Any]])-> Void) {
    threading.async {
      var rs: [String:[String:Any]] = [:]
      do {
        rs = try self.search(base: base, filter: filter, scope: scope, limit: limit)
      }catch {
        rs = [:]
      }//end catch
      completion(rs)
